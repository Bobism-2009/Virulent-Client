 ...
 *
 * Underground Mode: when the ceiling is lower than Fall Height, the fall is
 * accumulated over repeated small hops instead of one tall peak. The server
 * only sums downward deltas into fall distance (upward moves never reset it
 * while airborne), so many short up/down pairs inside the available headroom
 * reach the same fall distance as one open-sky peak without ever moving the
 * player into a block.
 */
public final class MaceKill extends Module {
	...
	/** Smallest hop worth sending; below this the headroom is unusable. */
	...
	public MaceKill() {
		super(
			"MaceKill",
			"Makes the Mace powerful when swung, even under low ceilings. Can also bypass totem usage.",
			Category.COMBAT,
	...
	private void handleAttack(ServerboundAttackPacket packet, CallbackInfo ci) {
		...
		double headroom = clearanceAbove(desiredFall);
		boolean confined = headroom < desiredFall;

		if (confined && undergroundMode.getValue()) {
			if (headroom < MIN_HOP) {
				feedback("No headroom at all - cannot spoof a fall here.");
				return;
			}
			double achievable = Math.min(desiredFall, headroom * maxHops.getValue().intValue());
			if (achievable < SMASH_THRESHOLD) {
				feedback("Ceiling too low to reach smash threshold (raise Max Hops).");
				return;
			}
		}
		...
		try {
			for (int i = 0; i < attackCount; i++) {
				double targetFall = desiredFall + i * heightIncrease.getValue().intValue();
				lastFall = spoofFall(targetFall, headroom, previousPos);
			...
			if (chatFeedback.getValue()) {
				String mode = confined && undergroundMode.getValue() ? " (underground)" : "";
				feedback("MaceKill x" + attackCount + " @ " + String.format("%.1f", lastFall) + " fall" + mode);
			}
		} finally {
	...
	/**
	 * Spoofs {@code targetFall} blocks of fall distance ending at {@code home}.
	 * In open space this is one peak + return, exactly like the legacy path.
	 * When headroom is short (and Underground Mode is on), the fall is split
	 * into up/down hops that each stay inside the free space above the player,
	 * since the server sums every downward delta into fall distance.
	 * Returns the fall distance actually accumulated.
	 */
	private double spoofFall(double targetFall, double headroom, Vec3 home) {
		boolean confined = headroom < targetFall;
		if (!confined || !undergroundMode.getValue()) {
			Vec3 peak = home.add(0, targetFall, 0);
			sendMove(peak);
			sendMove(home);
			return targetFall;
		}
		...
		int hops = 0;
		int hopLimit = maxHops.getValue().intValue();
		while (accumulated < targetFall && hops < hopLimit) {
			double hop = Math.min(headroom, targetFall - accumulated);
			if (hop < MIN_HOP) {
				break;
			}
			sendMove(home.add(0, hop, 0));
			sendMove(home);
			accumulated += hop;
			hops++;
		}
